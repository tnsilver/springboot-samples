...
class SortClassHelper {
   ...
   * @return
   */
  def sortClass(propertyName: String, sort: Sort): String = {
    if (propertyName == null || propertyName.isEmpty || !Option(sort).isDefined)
      return SORT_BOTH_CLASS
    val sortStr = sort.toString.toUpperCase()
