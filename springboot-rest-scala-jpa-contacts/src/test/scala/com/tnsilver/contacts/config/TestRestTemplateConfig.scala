...
/*
 * File: TestRestTemplateConfig.scala
 ...
 */
@Profile(Array("test"))
@Configuration class TestRestTemplateConfig {

  @Value("${spring.security.user.name}") private val user : String = null
  @Value("${spring.security.user.password}") private val password : String = null

  @Bean def testRestTemplate(restTemplateBuilder: RestTemplateBuilder): TestRestTemplate = { // @formatter:off
    restTemplateBuilder.additionalMessageConverters(util.List.of(new ByteArrayHttpMessageConverter, new StringHttpMessageConverter, new ResourceHttpMessageConverter, new AllEncompassingFormHttpMessageConverter, new MappingJackson2HttpMessageConverter))
    // @formatter:on
    ...
    restTemplateBuilder.additionalCustomizers((template: RestTemplate) => template.setRequestFactory(requestFactory))
    new TestRestTemplate(restTemplateBuilder).withBasicAuth(user, password)
  }
}
