    ...
    $.extend(
        altEditor.prototype,
        {
            ...
            _setup: function () {
                ...
                /*
                 * when the close button is hit, remove all 'deleting' classes
                 */
				$(this.modal_selector).on("hidden.bs.modal", function (e) {
					var adata = dt.rows(['.deleting','.editing','.adding']);
	                if (adata.count() !== 0) {
            ...
            _initLanguage: function () {
                ...
                this.language.error = {
                    ...
                    email: this.language.error.email || 'Expecting valid email address!'
                };
            },
            ...
            _editRowData: function () {
                ...
                var checkFilesQueued = function() {
                    if (numFilesQueued == 0) {
                        that.onEditRow(that, //alteditor, rowdata, success, error
                            rowDataArray,
            ...
            _openDeleteModal: function () {
                ...
                var adata = dt.rows({ selected: true });
                /*
                 * when clicking inline buttons in sub-tables,
                 * it does not mean the row was selected. For
                 * those cases a 'deleting' class can be added
                 * to the row's tr element in the onClick event
                 * handler, and then it can be trapped here.
                 */
                if (adata.count() === 0) {
					adata = dt.rows(['.deleting']);
				}
            ...
            _deleteRow: function () {
                ...
                if (adata.count() === 0) {
					adata = dt.rows(['.deleting']).data().toArray();
				} else {
					adata = dt.rows({ selected: true }).data().toArray();
				}
            ...
            completeColumnDefs: function () {
                ...
                for (var i in dt.context[0].aoColumns) {
                    ...
                    columnDefs[i] = {
                        ...
                        minLength: (obj.minLength ? obj.minLength : false),
                        size: (obj.size ? obj.size : false),
            ...
            */
                    ...
                    if (columnDefs[j].type.indexOf("hidden") >= 0) {
                        ...
                        else if (columnDefs[j].type.indexOf("textarea") >= 0)
                        {
                            data += "<textarea class='form-control' "
                                + "id='" + this._quoteattr(columnDefs[j].name) + "' "
                                + fillAttrs(columnDefs[j], ['name', 'style', 'rows', 'cols', 'maxLength','minLength', 'size', 'readonly', 'disabled', 'required'])
                                + "placeholder='" + this._quoteattr(columnDefs[j].placeholder ? columnDefs[j].placeholder : title) + "' "
                                + "data-special='" + this._quoteattr(columnDefs[j].special) + "' "
                                + "data-unique='" + columnDefs[j].unique + "'>"
                        ...
                        // Adding text-inputs and error labels, but also new HTML5 types (email, color, ...)
                            ...
                            data += "<input class='form-control' "
                                + fillAttrs(columnDefs[j], ['type', 'pattern', 'accept', 'name', 'step', 'min', 'max', 'maxLength', 'minLength', 'size', 'value', 'readonly', 'disabled', 'required'])
            ...
            _deleteRowCallback: function (response, status, more) {
                    ...
                    if (this.s.dt.rows({ selected : true }).any()) {
                    	this.s.dt.rows({ selected : true }).remove();
                    } else if (this.s.dt.rows(['.deleting']).any()) {
						this.s.dt.rows(['.deleting']).remove();
					}
                    this.s.dt.draw('page');
            ...
            _editRowCallback: function (response, status, more) {
                    ...
                    if (this.s.dt.row({selected: true}).any()) {
                    	this.s.dt.row({selected: true}).data(data);
                    } else {
						this.s.dt.row(['.editing']).data(data);
					}
            ...
            */
            ...
            /**
             * Accessor for indicating if this editor has an (server validation?) error
             */
            isInError: function() {
            ...
            /**
             * Method for setting an error message text inside the field label
             * with class 'errorLabel' and if neccessary, for marking this editor
             * as having an error.
			 * @param {string}
             *            fieldId the name of the field - must be an existing column definition 'name' attribute.
             * @param {string}
             *            errmsg (optional) the message to display in the error label. If empty or null, no errors will be marked on this editor.
             */
            setFieldError: function(fieldId, errmsg) {
				var columnDefs = this.completeColumnDefs();
				let found = false;
				let index = 0;
				for (; index < columnDefs.length; index++) {
					if (this._quoteattr(columnDefs[index].name) === fieldId) {
					  found = true;
					  break;
					}
				}
				if (found) {
					...
					const label_id = this._quoteattr(columnDefs[index].name) + "-label";
					const selector = "#" + modal_id + " #" + label_id + ".errorLabel";
					const target = $(selector);
					if (target) {
				  		target.html(errmsg !== (errmsg ?? {}) ? '' : errmsg);
				  		this._inError = errmsg !== (errmsg ?? {}) ? false : true;
					} else {
						console.error("dataTable.altEditor.free.js -> setFieldError('" + fieldId +"'): cannot find selector '" + selector + "'!");
					}
				} else {
					console.error("dataTable.altEditor.free.js -> setFieldError('" + fieldId +"'): cannot find column name '" + fieldId + "'!");
				}
            },

            /**
             * Convenience method for clearing an error message text inside a field label
             * with class 'errorLabel' and reseting the errors from this editor.
			 * @param {string}
             *            fieldId the name of the field - must be an existing column definition 'name' attribute.
             */
            ...
            /**
             * Convenience method for clearing a form error message
             */
            clearFormError: function() {
				const modal_id = 'altEditor-modal-' + this.random_id;
				const selector = '#' + modal_id + ' .alert.alert-danger';
				if (selector) {
					$(selector).remove();
				}
            }
